...
encrypt :: [Word8] -> [Word8]
encrypt extraSt = encryptAesEcb (pkcs7Padding ([1..5] ++ extraSt ++ toFind) 16) "YELLOW SUBMARINE"
...
breakOne' prefix it try skip = if isAesEcb' skip $ (encrypt ((replicate findExtraLength 0) ++ prefix ++ [try] ++ take (15 - it) prefix))
                       then try
...
findExtraLength' :: Int -> Int
findExtraLength' try = if isAesEcb $ encrypt (replicate (32 + try) 0)
                       then try
                       else findExtraLength' (try + 1)

findExtraLength :: Int
findExtraLength = findExtraLength' 0
